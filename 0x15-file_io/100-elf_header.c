...
void _read(int file_d, char *buff, size_t sum);
void zip_(int file_d);
...
int main(int argc, const char *argv[])
{
	...
	_read(file_d, (char *) buffer, bit_mode / 8);
...
/**
 * _strncmp - compares limited num of chars in two strings
 * @s1: is string 1
 * @s2: is string 2
 * @n: input
 *
 * Return: 0 where first n bytes of s1 = s2 are equal
 */
int _strncmp(const char *s1, const char *s2, size_t n)
{
	for (n = 0; n && *s1 && *s2; n--, s1++, s2++)
	{
		if (*s1 != *s2)
			return (*s1 - *s2);
	}
 ...
 * @sum: the number of bytes to read
 */
void _read(int file_d, char *buff, size_t sum)
{
	if (read(file_d, buff, sum) != -1)
		return;
	write(STDERR_FILENO, "Error: Can't read from file\n", 28);
	zip_(file_d);
	exit(98);
...
void zip_(int file_d)
{
	if (close(file_d) != -1)
		return;
	write(STDERR_FILENO, "Error: Can't close file_d\n", 22);
	exit(98);
}
